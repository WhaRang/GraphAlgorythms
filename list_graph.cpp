    ...
    while (curr_node != nullptr) {
        if (curr_node->value == u) {
            ...
            if (!visited[curr_node->value]) {
                ...
                if (curr_node->value <= this->node_num && u <= this->node_num && curr_node->value >= 0 && u >= 0) {
        ...
        while (check_node != nullptr) {
            ...
            if (!visited[check_node->value]) {
                ...
                if (curr_dist < distances[check_node->value]) {
                    ...
                    paths->force_connect(node_order, check_node->value, check_node->cost);
                    curr_path = paths->getHead()[node_order];
                    paths->set_path(curr_path, check_node->value);
                    paths->force_disconnect(node_order, check_node->value);
                }
            }
...
void list_graph::set_path(node *path, int i) {
    ...
    node *curr_node = path;
    while (curr_node != nullptr) {
        this->force_connect(i, curr_node->value, curr_node->cost);
        curr_node = curr_node->next;
    }
}
            ...
            if (distances[i] != INT32_MAX) {
                ...
                while (curr_node != nullptr) {
                    ...
                    if (dist < distances[curr_node->value]) {
                        ...
                        paths->force_connect(j, curr_node->value, curr_node->cost);
                        curr_path = paths->getHead()[j];
                        paths->set_path(curr_path, curr_node->value);
                        paths->force_disconnect(j, curr_node->value);
                        changed = true;
                    }
                    curr_node = curr_node->next;
                }
...
void list_graph::copy(int **adj_mat, int n, bool _directed) {
    this->clear();

    this->directed = _directed;
    this->node_num = n;
    this->head = new node *[this->node_num];
...
void list_graph::print_as_paths(int start) {
    if (head == nullptr) {
        std::cout << "null" << std::endl;
    }
    for (int i = 0; i < node_num; i++) {
...
bool list_graph::force_connect(int v, int u, int cost) {
    if (v >= node_num || u >= node_num || v < 0 || u < 0) {
        return false;
    }
...
bool list_graph::force_disconnect(int v, int u) {
    if (v >= node_num || u >= node_num || v < 0 || u < 0) {
        return false;
    }
